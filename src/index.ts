...
async function activateAdminDevice(request:Request,env:AdminEnv){
  const b=await readJson<{code:string;device_name?:string}>(request),code=cleanText(b.code,40).toUpperCase(),now=new Date().toISOString(),device=cleanText(b.device_name||'เครื่องแอดมิน',80),owner=await env.DB.prepare("SELECT id FROM admin_users WHERE role='main_owner' AND active=1").first();let adminId:string;
  if(!owner){if(code!==env.ADMIN_SETUP_CODE)throw new ApiError(403,'INVALID_SETUP_CODE','รหัสเปิดใช้งานไม่ถูกต้อง');adminId=crypto.randomUUID();await env.DB.prepare("INSERT INTO admin_users(id,display_name,role,created_at,updated_at) VALUES(?,'Owner หลัก','main_owner',?,?)").bind(adminId,now,now).run()}
  else{const invite=await env.DB.prepare("SELECT i.id,a.id admin_id FROM admin_invites i JOIN admin_users a ON a.id=i.admin_id WHERE i.code_hash=? AND i.used_at IS NULL AND i.expires_at>? AND a.active=1").bind(await hashToken(code),now).first<{id:string;admin_id:string}>();if(!invite)throw new ApiError(403,'INVALID_INVITE_CODE','รหัสเชื่อมเครื่องไม่ถูกต้อง หมดอายุ หรือถูกใช้แล้ว');adminId=invite.admin_id;await env.DB.prepare('UPDATE admin_invites SET used_at=? WHERE id=? AND used_at IS NULL').bind(now,invite.id).run()}
  const token=crypto.randomUUID()+crypto.randomUUID();await env.DB.prepare('INSERT INTO admin_devices(id,admin_id,token_hash,device_name,last_seen_at,created_at) VALUES(?,?,?,?,?,?)').bind(crypto.randomUUID(),adminId,await hashToken(token),device,now,now).run();return new Response(JSON.stringify({ok:true}),{headers:{'content-type':'application/json','set-cookie':`ds_admin_device=${token}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=31536000`}})
}
...
async function adminBootstrap(env:Env,identity:AdminSession):Promise<Response>{
  ...
  const [products,orders,customers,payments,closings,totalProducts,lowStock,totalCustomers,totalOrders,todayOrders,todaySales]=await Promise.all([
    ...
    env.DB.prepare(`SELECT c.id,c.name,c.phone,c.address,c.points,c.verified,c.active,c.deleted_at,c.created_at,c.updated_at,a.id admin_user_id,a.role admin_role,a.commission_percent admin_commission,a.active admin_active FROM customers c LEFT JOIN admin_users a ON a.customer_id=c.id ORDER BY c.active DESC,c.updated_at DESC LIMIT 150`).all(),
...
function newInviteCode(){const alphabet='ABCDEFGHJKLMNPQRSTUVWXYZ23456789',bytes=crypto.getRandomValues(new Uint8Array(12));return [...bytes].map(x=>alphabet[x%alphabet.length]).join('')}
async function promoteCustomerToAdmin(request:Request,env:Env,owner:AdminSession,customerId:string){
  const b=await readJson<{role?:string;commission_percent?:number}>(request),role=cleanText(b.role||'admin',20),commission=Number(b.commission_percent||0);if(!['main_owner','co_owner'].includes(owner.role))throw new ApiError(403,'OWNER_REQUIRED','เฉพาะ Owner เท่านั้นที่เพิ่มแอดมินได้');if(!['admin','co_owner'].includes(role))throw new ApiError(400,'INVALID_ADMIN_ROLE','เลือกสิทธิ์แอดมินหรือเจ้าของรองเท่านั้น');if(role==='co_owner'&&owner.role!=='main_owner')throw new ApiError(403,'MAIN_OWNER_REQUIRED','เฉพาะเจ้าของหลักเท่านั้นที่เพิ่มเจ้าของรองได้');if(!Number.isFinite(commission)||commission<0||commission>100)throw new ApiError(400,'INVALID_COMMISSION','ค่าคอมต้องอยู่ระหว่าง 0 ถึง 100');
  const customer=await env.DB.prepare('SELECT id,name,active FROM customers WHERE id=?').bind(customerId).first<{id:string;name:string;active:number}>();if(!customer||!customer.active)throw new ApiError(404,'CUSTOMER_NOT_ACTIVE','ไม่พบลูกค้าที่เปิดใช้งาน');const existing=await env.DB.prepare('SELECT id,role FROM admin_users WHERE customer_id=?').bind(customerId).first<{id:string;role:string}>();if(role==='co_owner'){const count=await env.DB.prepare("SELECT COUNT(*) value FROM admin_users WHERE role='co_owner' AND active=1 AND id<>?").bind(existing?.id||'').first<{value:number}>();if((count?.value||0)>=3)throw new ApiError(409,'CO_OWNER_LIMIT','เจ้าของรองครบ 3 คนแล้ว กรุณาปิดสิทธิ์คนเดิมก่อน')}
  const now=new Date(),adminId=existing?.id||crypto.randomUUID(),code=newInviteCode(),expires=new Date(now.getTime()+24*60*60*1000).toISOString();if(existing)await env.DB.prepare('UPDATE admin_users SET display_name=?,role=?,commission_percent=?,active=1,deleted_at=NULL,updated_at=? WHERE id=?').bind(customer.name,role,commission,now.toISOString(),adminId).run();else await env.DB.prepare('INSERT INTO admin_users(id,display_name,role,commission_percent,active,customer_id,created_at,updated_at) VALUES(?,?,?,?,1,?,?,?)').bind(adminId,customer.name,role,commission,customerId,now.toISOString(),now.toISOString()).run();await env.DB.batch([env.DB.prepare('UPDATE admin_invites SET used_at=? WHERE admin_id=? AND used_at IS NULL').bind(now.toISOString(),adminId),env.DB.prepare('INSERT INTO admin_invites(id,admin_id,code_hash,created_by,expires_at,created_at) VALUES(?,?,?,?,?,?)').bind(crypto.randomUUID(),adminId,await hashToken(code),owner.id,expires,now.toISOString())]);await writeAudit(env,owner,existing?'reactivate_admin':'create_admin','admin_user',adminId,{customer_id:customerId,display_name:customer.name,role,commission_percent:commission});return json({ok:true,admin_user_id:adminId,display_name:customer.name,role,commission_percent:commission,activation_code:code,expires_at:expires});
}
...
export default {
  async fetch(request:Request,env:Env,ctx:ExecutionContext):Promise<Response>{
    ...
    try{
      if(request.method==='POST'&&url.pathname==='/api/admin/device/activate')return await activateAdminDevice(request,env as AdminEnv);
      if(url.pathname.startsWith('/api/admin/')){
        ...
        const promoteCustomer=url.pathname.match(/^\/api\/admin\/customers\/([^/]+)\/promote$/);
        if(request.method==='POST'&&promoteCustomer)return await promoteCustomerToAdmin(request,env,identity,decodeURIComponent(promoteCustomer[1]));
